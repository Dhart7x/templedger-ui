        ...
        <div className="space-y-2 md:space-y-4">
          {features.map((feature, index) => (
            <motion.div
              ...
              className="flex items-start gap-2 md:gap-3 p-2 md:p-4 rounded-lg bg-card border border-border"
            >
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ delay: 0.4 + index * 0.1, type: "spring", stiffness: 300 }}
                className="w-7 h-7 md:w-9 md:h-9 rounded-full bg-primary flex items-center justify-center flex-shrink-0"
              >
                <feature.icon className="w-3.5 h-3.5 md:w-5 md:h-5 text-primary-foreground" />
              </motion.div>
