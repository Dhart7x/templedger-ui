...
const DemoAgencyIssues = () => {
  const { exceptions } = useDemoContext();
  ...
  const getResolutionStatus = (issueId: string) => {
    const exception = exceptions.find(e => e.id === issueId);
            ...
            <div className="space-y-2">
              {criticalIssues.map((issue) => {
                ...
                return (
                  <div
                    key={issue.id}
                    className="bg-card border border-destructive/30 rounded-lg p-4 cursor-pointer hover:bg-muted/30 transition-colors"
                    onClick={() => {
                      if (!resolution) {
                        setSelectedIssue({
                          id: issue.id,
                      ...
                      <div className="p-2 bg-emerald-500/10 rounded-lg border border-emerald-500/20">
                        {resolution.resolutionType === "on-the-way" ? (
                          ...
                          <div className="flex items-center gap-2 text-sm text-emerald-500">
                            ...
                            Replaced by {resolution.replacementWorkerName} — ETA {resolution.replacementEtaMinutes} mins
                          </div>
                        )}
                      </div>
                    ) : (
            ...
            <div className="space-y-2">
              {warningIssues.map((issue) => {
                ...
                return (
                  <div
                    key={issue.id}
                    className="bg-card border border-amber-500/30 rounded-lg p-4 cursor-pointer hover:bg-muted/30 transition-colors"
                    onClick={() => {
                      if (!resolution) {
                        setSelectedIssue({
                          id: issue.id,
