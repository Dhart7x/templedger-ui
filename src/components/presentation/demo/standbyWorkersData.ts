 ...
 const departments = ["Picking", "Packing", "Goods In", "Returns", "Loading", "Warehouse", "Quality"];
 ...
 const generateWorker = (index: number): StandbyWorker => {
   ...
   const isLive = index < 8; // First 8 are live workers
   ...
   return {
     ...
     departments: [bestDept, departments[(index + 2) % departments.length]].filter((v, i, a) => a.indexOf(v) === i),
     preferences: {
     ...
     experience: [
       ...
       ...(index % 3 === 0 ? [{ role: departments[(index + 3) % departments.length], years: Math.floor(Math.random() * 3) + 1 }] : []),
     ],
